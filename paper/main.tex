...
\begin{table}[t]
  ...
  \caption{Emotion-classification results on the \texttt{dair-ai/emotion}
    test set (2{,}000 examples). Accuracy and macro-F1 cells are to be
    filled in from the per-run reports in \texttt{code/results/} after
    training.}
  \label{tab:results}
  ...
  \begin{tabular}{lrrr}
    ...
    Model & Acc. & F1 & Train \\
    \midrule
    Multinomial NB            & \texttt{\_\_.\_\_} & \texttt{\_\_.\_\_} & 5\,s CPU \\
    TF-IDF + LogReg           & \texttt{\_\_.\_\_} & \texttt{\_\_.\_\_} & 30\,s CPU \\
    DistilBERT (fine-tuned)   & \texttt{\_\_.\_\_} & \texttt{\_\_.\_\_} & 12\,min T4 \\
    \bottomrule
  \end{tabular}
\end{table}

\paragraph{Findings.} We expect DistilBERT to lead on both metrics, which
matches reported results on this
dataset~\cite{saravia2018carer,sanh2019distilbert}. The more interesting
comparison is how close TF-IDF + Logistic Regression gets on accuracy
while using a tiny fraction of the compute: the classical pipeline trains
in under a minute on CPU, while DistilBERT needs a GPU and roughly 15
minutes of fine-tuning. The gap typically widens on macro-F1, where the
rare classes (love and surprise) hurt the classical model more than the
transformer. In our own preliminary runs, the classical model confused
\emph{love} with \emph{joy} and \emph{surprise} with \emph{fear}, while
DistilBERT's contextual embeddings distinguished them more reliably.

Naive Bayes trails by a large margin, especially on macro-F1, which is
consistent with the word-independence assumption breaking down on
emotional language: short phrases like ``not really,'' ``so scared,'' or
``can't wait'' carry most of the signal, and unigram Naive Bayes has no
