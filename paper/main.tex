...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{distilbert_cm.pdf}
  \caption{Confusion matrix for fine-tuned DistilBERT on the
    \texttt{dair-ai/emotion} test set. Rows are true labels and columns
    are predictions; diagonal cells are correct classifications. Even
    the rare classes \emph{love} and \emph{surprise} are classified
    correctly in most cases, and the largest remaining confusion for
    \emph{love} is with its semantically closest neighbor,
    \emph{joy}.}
  \label{fig:cm}
\end{figure}
...
of training examples), DistilBERT achieves an F1 of 0.80
(\cref{fig:cm}), compared to just 0.42 for TF-IDF $+$ Logistic
Regression and 0.21 for Naive Bayes. The trade-off is compute: the
classical pipeline trains in under a minute on a laptop CPU, while
DistilBERT requires a GPU and roughly 12 minutes of fine-tuning. For
applications where a few points of accuracy are acceptable, the
classical pipeline remains an attractive, interpretable choice.
